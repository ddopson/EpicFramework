...
package com.epic.framework.implementation.util.billing;

import com.epic.framework.common.util.EpicLog;
import com.epic.framework.implementation.util.billing.EpicBillingConstants.ResponseCode;
import com.epic.framework.implementation.util.billing.EpicBillingSecurityHelpers.VerifiedPurchase;
 ...
 *
 * The {@link EpicBillingReceiver} class starts this service to process commands
 * that it receives from Android Market.
 *
 * You should modify and obfuscate this code before using it.
 */
public class EpicInAppService extends Service implements ServiceConnection {
    ...
    abstract class BillingRequest {
        ...
        public boolean runIfConnected() {
            if (EpicBillingConstants.DEBUG) {
                EpicLog.d(getClass().getSimpleName());
            }
            if (mService != null) {
                try {
                    mRequestId = run();
                    if (EpicBillingConstants.DEBUG) {
                        EpicLog.d("request id: " + mRequestId);
                    }
                    if (mRequestId >= 0) {
        ...
        protected void onRemoteException(RemoteException e) {
            EpicLog.w("remote billing service crashed");
            mService = null;
        }
        ...
        protected Bundle makeRequestBundle(String method) {
            ...
            request.putString(EpicBillingConstants.BILLING_REQUEST_METHOD, method);
            request.putInt(EpicBillingConstants.BILLING_REQUEST_API_VERSION, 1);
            request.putString(EpicBillingConstants.BILLING_REQUEST_PACKAGE_NAME, getPackageName());
            return request;
        }

        protected void logResponseCode(String method, Bundle response) {
            ResponseCode responseCode = ResponseCode.valueOf(
                    response.getInt(EpicBillingConstants.BILLING_RESPONSE_RESPONSE_CODE));
            if (EpicBillingConstants.DEBUG) {
                EpicLog.e(method + " received " + responseCode.toString());
            }
        }
    }
    ...
    class CheckBillingSupported extends BillingRequest {
        ...
        protected long run() throws RemoteException {
            ...
            if (EpicBillingConstants.DEBUG) {
                EpicLog.i("CheckBillingSupported response code: " +
    ...
    class RequestPurchase extends BillingRequest {
        ...
        protected long run() throws RemoteException {
            ...
            // TODO: change this back to mProductId
            request.putString(EpicBillingConstants.BILLING_REQUEST_ITEM_ID, mProductId);
            // Note that the developer payload is optional.
            if (mDeveloperPayload != null) {
                request.putString(EpicBillingConstants.BILLING_REQUEST_DEVELOPER_PAYLOAD, mDeveloperPayload);
            }
            Bundle response = mService.sendBillingRequest(request);
            PendingIntent pendingIntent
                    = response.getParcelable(EpicBillingConstants.BILLING_RESPONSE_PURCHASE_INTENT);
            if (pendingIntent == null) {
                EpicLog.e("Error with requestPurchase");
                return EpicBillingConstants.BILLING_RESPONSE_INVALID_REQUEST_ID;
            }

            Intent intent = new Intent();
            EpicBillingResponseHandler.buyPageIntentResponse(pendingIntent, intent);
            return response.getLong(EpicBillingConstants.BILLING_RESPONSE_REQUEST_ID,
                    EpicBillingConstants.BILLING_RESPONSE_INVALID_REQUEST_ID);
        }

        @Override
        protected void responseCodeReceived(ResponseCode responseCode) {
            EpicBillingResponseHandler.responseCodeReceived(EpicInAppService.this, this, responseCode);
        }
    }
    ...
    class ConfirmNotifications extends BillingRequest {
        ...
        protected long run() throws RemoteException {
            ...
            request.putStringArray(EpicBillingConstants.BILLING_REQUEST_NOTIFY_IDS, mNotifyIds);
            Bundle response = mService.sendBillingRequest(request);
    ...
    class GetPurchaseInformation extends BillingRequest {
        ...
        @Override
        protected long run() throws RemoteException {
            mNonce = EpicBillingSecurityHelpers.generateNonce();

            Bundle request = makeRequestBundle("GET_PURCHASE_INFORMATION");
            request.putLong(EpicBillingConstants.BILLING_REQUEST_NONCE, mNonce);
            request.putStringArray(EpicBillingConstants.BILLING_REQUEST_NOTIFY_IDS, mNotifyIds);
            Bundle response = mService.sendBillingRequest(request);
        ...
        protected void onRemoteException(RemoteException e) {
            ...
            EpicBillingSecurityHelpers.removeNonce(mNonce);
        }
    }
    ...
    /**
     * The {@link EpicBillingReceiver} sends messages to this service using intents.
     * Each intent has an action and some extra arguments specific to that action.
     * @param intent the intent containing one of the supported actions
     * @param startId an identifier for the invocation instance of this service
     */
    public void handleCommand(Intent intent, int startId) {
    	// TODO: HUGE BUG FARM--WHEN I RAN, INTENT WAS NULL. UNBLOCKING FOR NOW.
    	if(intent == null) return;
        String action = intent.getAction();
        if (EpicBillingConstants.DEBUG) {
            EpicLog.i("handleCommand() action: " + action);
        }
        if (EpicBillingConstants.ACTION_CONFIRM_NOTIFICATION.equals(action)) {
            String[] notifyIds = intent.getStringArrayExtra(EpicBillingConstants.NOTIFICATION_ID);
            confirmNotifications(startId, notifyIds);
        } else if (EpicBillingConstants.ACTION_GET_PURCHASE_INFORMATION.equals(action)) {
            String notifyId = intent.getStringExtra(EpicBillingConstants.NOTIFICATION_ID);
            getPurchaseInformation(startId, new String[] { notifyId });
        } else if (EpicBillingConstants.ACTION_PURCHASE_STATE_CHANGED.equals(action)) {
    ...
    private boolean bindToMarketBillingService() {
        try {
            if (EpicBillingConstants.DEBUG) {
                EpicLog.i("binding to Market billing service");
            }
            boolean bindResult = bindService(
                    new Intent(EpicBillingConstants.MARKET_BILLING_SERVICE_ACTION),
            ...
            if (bindResult) {
                ...
                EpicLog.e("Could not bind to service.");
            }
     ...
     * the purchase succeeds (or is canceled) the {@link EpicBillingReceiver}
     * receives an intent with the action {@link EpicBillingConstants#ACTION_NOTIFY}.
     ...
     * encoded as a JSON string, and sends that to the {@link EpicBillingReceiver}
     * in an intent with the action {@link EpicBillingConstants#ACTION_PURCHASE_STATE_CHANGED}.
     ...
     * {@link EpicBillingResponseHandler#purchaseResponse(Context, PurchaseState, String, String, long)}
    ...
    private void purchaseStateChanged(int startId, String signedData, String signature) {
        ArrayList<EpicBillingSecurityHelpers.VerifiedPurchase> purchases;
        purchases = EpicBillingSecurityHelpers.verifyPurchase(signedData, signature);
        if (purchases == null) {
            return;
        }
        ...
        for (VerifiedPurchase vp : purchases) {
            ...
            EpicBillingResponseHandler.purchaseResponse(this, vp.purchaseState, vp.productId,
     ...
     * in the {@link EpicBillingReceiver} and passed to this service, where they are
    ...
    private void checkResponseCode(long requestId, ResponseCode responseCode) {
        ...
        if (request != null) {
            if (EpicBillingConstants.DEBUG) {
                EpicLog.d(request.getClass().getSimpleName() + ": " + responseCode);
            }
    ...
    private void runPendingRequests() {
        ...
        if (maxStartId >= 0) {
            if (EpicBillingConstants.DEBUG) {
                EpicLog.i("stopping service, startId: " + maxStartId);
            }
            stopSelf(maxStartId);
        }
    }
    ...
    public void onServiceConnected(ComponentName name, IBinder service) {
        if (EpicBillingConstants.DEBUG) {
            EpicLog.d("Billing service connected");
        }
    ...
    public void onServiceDisconnected(ComponentName name) {
        EpicLog.w("Billing service disconnected");
        mService = null;
    }
